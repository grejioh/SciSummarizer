...
class PDFProcessor:
    ...
    def extract_text_from_page(self, pdf_path: str, page_number: int) -> Optional[str]:
        ...
        try:
            self.logger.info(
                f"Extracting text from page {page_number} of PDF: {pdf_path}"
            )
            with open(pdf_path, "rb") as file:
                reader = PyPDF2.PdfReader(file)
            ...
            self.logger.info(
                f"Successfully extracted {len(text)} characters from page {page_number} of {pdf_path}"
            )
            return text.strip()
        except Exception as e:
            self.logger.error(
                f"Error extracting text from page {page_number} of {pdf_path}: {str(e)}"
            )
            return None

    def extract_text_from_fisrt_N_pages(
        self, pdf_path: str, max_pages_num: int
    ) -> Optional[str]:
        try:
            text = ""
            ...
            self.logger.info(
                f"Successfully extracted {len(text)} characters from the first {max_pages_num} pages of {pdf_path}"
            )
            return text.strip()
        except Exception as e:
            self.logger.error(
                f"Error extracting text from first {page_number} pages of {pdf_path}: {str(e)}"
            )
            return None
...
if __name__ == "__main__":
    ...
    page_text = processor.extract_text_from_page(
        pdf_path, 0
    )  # Extract text from the first page
    if page_text:
        print(
            f"Extracted text from first page (first 100 characters): {page_text[:100]}..."
        )
